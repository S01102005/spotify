...
const tracks = [
    {
        ...
        album: "Mere Paas Tum Ho (Original Soundtrack)",
        duration: "9:36", 
    ...
    },
    {
        ...
        artist: "Bhikari Bal",
        album: "Sathie Pauti Bhogaru Tumara(original)",
        duration: "6:24",
    ...
    },
    {
        ...
        artist: "Humane Sagar",
        album: "Mu Ta Bada Deulara Parare(original)",
        duration: "6:33",
    ...
    },
    {
        ...
        album: "Yeh Raaten Yeh Mausam(original)",
        duration: "3:39",
    ...
    },
    {
        ...
        album: "O Haseena Zulfonwali(original)",
        duration: "5:05",
    ...
    },
    {
        ...
        artist: "Arijit Singh",
        album: "Tujhe Kitna Chahne Lage(original)",
        duration: "4:44",
    ...
    },
    {
        ...
        album: "Bol Na Halke Halke(original)",
        duration: "7:41",
    ...
    },
    {
        ...
        album: "Illegal Weapon 2.0(original)",
        duration: "3:08",
    ...
    },
    {
        ...
        album: "Kar Har Maidan Fateh(original)",
        duration: "5:11",
...
function initializePlayer() {
    ...
    // Initialize Queue and Device buttons
    initializeQueueAndDeviceButtons();
}
...
function loadTrack(index) {
    ...
    if (totTimeEl) totTimeEl.textContent = track.duration;
...
function goToHome() {
    ...
    // Use showHomePage to properly display home content
    showHomePage();
}
...
// Apply audio quality setting (adjusts volume)
...
// Queue and Device Buttons Functionality
function initializeQueueAndDeviceButtons() {
...
function showQueueModal() {
    ...
    modal.innerHTML = `
        <div class="modal-content">
            <div class="modal-header">
                <h2>Queue</h2>
                <button class="modal-close-btn" id="closeQueueModalBtn">
                    <i class="fa-solid fa-times"></i>
                </button>
            </div>
    ...
    // Close modal handlers
    const closeBtn = modal.querySelector('#closeQueueModalBtn');
    closeBtn.addEventListener('click', () => modal.remove());
    modal.addEventListener('click', (e) => {
        if (e.target === modal) modal.remove();
    });
    ...
    const queueItems = modal.querySelectorAll('.queue-item');
...
function renderQueueList() {
    if (tracks.length === 0) {
        return '<div class="empty-queue"><p>No songs in queue</p></div>';
    }
    
    // Show current track and next tracks
    const currentIndex = currentTrackIndex;
    const queueTracks = [];
    
    // Add remaining tracks from current position
    for (let i = currentIndex; i < tracks.length; i++) {
        queueTracks.push({ track: tracks[i], index: i, isCurrent: i === currentIndex });
    }
    
    // Add tracks from beginning if needed
    if (queueTracks.length < 10) {
        for (let i = 0; i < currentIndex && queueTracks.length < 10; i++) {
            queueTracks.push({ track: tracks[i], index: i, isCurrent: false });
        }
    }
    
    return queueTracks.map((item, idx) => `
        <div class="queue-item ${item.isCurrent ? 'current' : ''}" data-track-index="${item.index}">
            <div class="queue-item-number">${idx + 1}</div>
            <img src="${item.track.image}" alt="${item.track.title}" class="queue-item-image">
            <div class="queue-item-info">
                <div class="queue-item-title">${item.track.title}</div>
                <div class="queue-item-artist">${item.track.artist}</div>
            </div>
            ${item.isCurrent ? '<span class="queue-current-badge">Now Playing</span>' : ''}
        </div>
    `).join('');
}
...
function showDeviceModal() {
    ...
    modal.innerHTML = `
        <div class="modal-content">
            <div class="modal-header">
                ...
                <button class="modal-close-btn" id="closeDeviceModalBtn">
                    <i class="fa-solid fa-times"></i>
                </button>
            </div>
            <div class="device-list" id="deviceList">
                <div class="device-item active">
                    ...
                    <div class="device-info">
                        <div class="device-name">This Computer</div>
                        <div class="device-status">Current device</div>
                    </div>
                    <i class="fa-solid fa-check device-active-icon"></i>
                </div>
            ...
            <div class="device-footer">
                <p class="device-hint">Connect to other devices to play music remotely</p>
            </div>
        </div>
    `;
    ...
    // Close modal handlers
    const closeBtn = modal.querySelector('#closeDeviceModalBtn');
    closeBtn.addEventListener('click', () => modal.remove());
    modal.addEventListener('click', (e) => {
        if (e.target === modal) modal.remove();
    });
    ...
    const deviceItems = modal.querySelectorAll('.device-item:not(.active)');
    deviceItems.forEach(item => {
        item.addEventListener('click', () => {
            alert('Device connection feature requires Spotify integration. This is a demo feature.');
        });
    });
}
...
function initializeSplashScreen() {
    ...
    // Step 1: Background image (bg_white.jpg) fade-in after 1.5 seconds
    setTimeout(() => {
        if (splashBg) {
    ...
    // Step 2: Logo (spott.jpg) fade-in after 0.5 seconds from background (2 seconds total)
    setTimeout(() => {
        if (splashLogo) {
    ...
    // Step 3: Text "SPOTIFY" fade-in after 1 second from logo (3 seconds total)
    setTimeout(() => {
        if (splashText) {
    ...
    // Step 4: Full Splash Screen Fade-out (after 5 seconds total)
    setTimeout(() => {
        splashScreen.classList.add('fade-out');
        if (splashBg) {
            splashBg.classList.remove('visible');
        }
        if (splashLogo) {
    ...
    // Step 5: Remove Splash Screen from DOM (after fade-out completes = ~6 seconds total)
    setTimeout(() => {
        splashScreen.style.display = 'none';
    }, 6000);
}
if (recentlyPlayedContainer) {
    // Clear any existing HTML inside the container
    recentlyPlayedContainer.innerHTML = ''; 

    // Loop through every song object in your tracks array
    tracks.forEach((track, index) => {
        // Create the necessary HTML structure
        const cardDiv = document.createElement('div');
        cardDiv.classList.add('card');
        cardDiv.dataset.trackIndex = index; // Store the index for playback

        // Fill the card with data from the 'track' object
        cardDiv.innerHTML = `
        ...
        // Add the new card to the homepage container
        recentlyPlayedContainer.appendChild(cardDiv);
        
        // Add the click listener: clicking the card loads and plays the song
        cardDiv.addEventListener('click', () => {
            loadTrack(index); 
            // Start playing the song automatically
            if (!isPlaying) { 
                playPause(); // Call your existing playPause function 
            } 
        });
    });
}
